...
echo zkGenConfig.sh Params $1 $(id -u) "$ZK_USER" "$ZK_DATA_DIR" "$ZK_DATA_LOG_DIR" "$ZK_LOG_DIR" 
if [[ "$1" = 'zkServer.sh' && "$(id -u)" = '0' ]]; then
    chown -R "$ZK_USER" "$ZK_DATA_DIR" "$ZK_DATA_LOG_DIR" "$ZK_LOG_DIR" 
...
HOST=`echo $K8S_POD_ID`
DOMAIN=`echo zk.$K8S_NAMESPACE.svc.cluster.local`
